...
from impulse.EMAIL.email_tools import ReadSenderEmail
...
def __GetEmailMessage():
    ...
    subject = input(
        f"{Fore.BLUE}[?] {Fore.MAGENTA}Enter the Subject (leave blank for random value): ",
    )
    body = input(
        f"{Fore.BLUE}[?] {Fore.MAGENTA}Enter Your Message (leave blank for random value): ",
    )
...
def GetTargetAddress(target, method):
    ...
    if method in (
        ...
        return origin, 80
    if method in ("SYN", "UDP", "NTP", "POD", "MEMCACHED", "ICMP", "SLOWLORIS"):
